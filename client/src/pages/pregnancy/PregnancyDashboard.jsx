...
import motherImg from "../../assets/mother.png"; // Import mother image
...
export default function PregnancyDashboard() {
  ...
  // Check if active pregnancy exists
  const hasPregnancy = Boolean(data && data.pregnancy);
  ...
  return (
    <PageLayout title="Pregnancy Dashboard" subtitle="Track your pregnancy journey and stay healthy.">
      {/* ------------------------------------------------------------- */}
      {/* CONDITION 1: NO LOGGED PREGNANCY -> SHOW START TRACKING CALLOUT */}
      {/* ------------------------------------------------------------- */}
      {!hasPregnancy ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow-[0_4px_14px_rgba(0,0,0,0.04)] ring-1 ring-black/5 sm:p-10">
          ...
          <h2 className="font-display text-xl font-semibold text-[#0D0D0D]">
            No Active Pregnancy Tracked
          </h2>
          <p className="mx-auto mt-1 max-w-sm text-sm text-[#8F8C8C]">
            Start tracking your pregnancy journey to calculate your due date, receive weekly baby updates, and manage health reminders.
          </p>
          <div className="mt-6 flex justify-center">
            <button
              onClick={() => navigate("/pregnancy/update")}
              className="inline-flex items-center gap-2 rounded-full bg-[#F33B7D] px-6 py-3 text-sm font-semibold text-white shadow-[0_14px_28px_-6px_rgba(243,59,125,0.5)] transition hover:-translate-y-0.5"
            >
