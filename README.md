# Flora — Women’s Gynae Health Platform

**Flora** is an AI-powered women’s gynecological health platform designed to provide accessible, secure, and culturally appropriate healthcare support for women, with a focus on the needs of women in Pakistan.

The platform brings essential gynecological and reproductive health services together in one place, including **menstrual cycle tracking, PCOS risk assessment, medical report analysis, pregnancy guidance, doctor–patient communication, health education, dietary management, and maternity exercise guidance**.

Flora supports **English and Urdu** to improve accessibility and provide health information in a more understandable and culturally relevant way.

> **Disclaimer:** Flora is an academic Final Year Project. AI-based features provide risk indications and informational guidance only and are **not intended to replace professional medical diagnosis or treatment**.


## System Architecture

Flora is designed as a multi-component system consisting of:

```text
                    ┌──────────────────────┐
                    │     Flora Client     │
                    │  Web / Mobile App    │
                    └──────────┬───────────┘
                               │
                               ▼
                    ┌──────────────────────┐
                    │      REST API        │
                    │   Node.js + Express  │
                    └──────────┬───────────┘
                               │
             ┌─────────────────┼─────────────────┐
             ▼                 ▼                 ▼
      ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
      │   MongoDB   │   │  AI / ML    │   │   Services  │
      │   Database  │   │   Python    │   │ OCR / NLP   │
      └─────────────┘   └─────────────┘   └─────────────┘
```

## Security & Privacy

Because Flora handles sensitive health information, security and privacy are important parts of the system.
Flora follows **HIPAA-inspired security and privacy practices** for academic purposes. It is **not HIPAA-certified** and is not intended for clinical deployment yet.

---

## Project Team

| Member                 | 
| ---------------------- | 
| **Khansa Waheed**      |         
| **Momna Nawaz Abbasi** |         
| **Shiza Riaz**         |      


## Development Status

Flora is currently under active development as a **Final Year Project (FYP)** at Air University Islamabad.

---

## Academic Project

Flora is developed as a **Final Year Project (FYP)** by students of the Department of Computer Science at **Air University Islamabad**.

The project focuses on combining **software engineering, artificial intelligence, healthcare technology, and user-centered design** to create an integrated women's health platform.

---

## License

This project is developed for **academic and educational purposes** as part of a Final Year Project.

---

<p align="center">
  Made with ❤️ by the Flora Team
</p>

